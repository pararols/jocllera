...
// Constants for Physics & Game Logic
const GRID_SIZE = 15;
const WIN_PERCENT = 90;
const LOSS_PERCENT = 90;
const GRAVITY = 0.025;

const MobileAquiferDefense = ({ onComplete, onClose, onImpact, onPenalty }) => {
    ...
    const [windowSize] = useState({ w: window.innerWidth, h: window.innerHeight });

    // Dynamic Size for Mobile
    const CANVAS_WIDTH = windowSize.w;
    const CANVAS_HEIGHT = windowSize.h * 0.85; // Leave top space for HUD
    const ZONE_WIDTH = CANVAS_WIDTH * 0.28;
    const BOTTOM_HEIGHT = CANVAS_HEIGHT * 0.25;
    const SIDE_TOP_MARGIN = 60;
    ...
    return ReactDOM.createPortal(
        <div className="mobile-minigame-overlay" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100dvh', background: '#000', zIndex: 10000, display: 'flex', flexDirection: 'column' }}>

            {/* Header */}
            <div style={{ padding: '15px 20px', background: 'rgba(15, 23, 42, 1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, zIndex: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <Waves color="#3498db" size={24} />
                    <h3 style={{ margin: 0, fontSize: '1.1rem', color: 'white' }}>Defensa de l'Aqüífer</h3>
                </div>
            ...
            <div style={{ position: 'relative', width: '100%', flex: 1, touchAction: 'none', backgroundColor: '#000', overflow: 'hidden' }}>
                <canvas
                ...
                <div style={{ position: 'absolute', top: 10, left: 10, right: 10, display: 'flex', justifyContent: 'space-between', pointerEvents: 'none' }}>
                    <div style={{ color: '#2ecc71', backgroundColor: 'rgba(0,0,0,0.8)', padding: '6px 12px', borderRadius: '15px', border: '2px solid #2ecc71', fontWeight: 'bold', fontSize: '0.9rem' }}>
                        GRANGES: {savedPercent}%
                    </div>
                    <div style={{ color: '#e74c3c', backgroundColor: 'rgba(0,0,0,0.8)', padding: '6px 12px', borderRadius: '15px', border: '2px solid #e74c3c', fontWeight: 'bold', fontSize: '0.9rem' }}>
                        GOLF: {lostPercent}%
                    </div>
                </div>

                <div style={{ position: 'absolute', bottom: 20, left: 0, right: 0, color: 'rgba(255,255,255,0.7)', textAlign: 'center', fontSize: '0.85rem', pointerEvents: 'none', background: 'rgba(0,0,0,0.5)', padding: '10px 0' }}>
                    Dibuixa línies per desviar les gotes
                </div>

                {gameState === 'success' && (
                    <div className="fade-in" style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(15, 23, 42, 0.95)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 20, textAlign: 'center', padding: '20px' }}>
                        <ShieldCheck size={100} color="#2ecc71" style={{ filter: 'drop-shadow(0 0 20px #2ecc71)', marginBottom: '20px' }} />
                        <h2 style={{ color: '#2ecc71', fontSize: '2rem', margin: '0 0 10px 0' }}>L'AIGUA HA TORNAT!</h2>
                        <p style={{ color: 'white', fontSize: '1.1rem' }}>Has restaurat el bosc de ribera i l'agricultura local.</p>
                    </div>
                )}

                {gameState === 'failure' && (
                    <div className="fade-in" style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(20, 10, 10, 0.95)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 20, pointerEvents: 'auto', textAlign: 'center', padding: '20px' }}>
                        <AlertCircle size={100} color="#e74c3c" style={{ filter: 'drop-shadow(0 0 20px #e74c3c)', marginBottom: '20px' }} />
                        <h2 style={{ color: '#e74c3c', fontSize: '2rem', margin: '0 0 10px 0' }}>AQÜÍFER ESPLUAT</h2>
                        <p style={{ color: 'white', fontSize: '1.1rem' }}>La gespa artificial ha vençut.</p>
                        <button
                            onClick={() => {
                            ...
                            style={{ padding: '15px 30px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', fontSize: '1.1rem', marginTop: '30px', width: '100%', maxWidth: '300px' }}
                        >REINTENTAR</button>
                    </div>
                )}
            </div>
...
export default MobileAquiferDefense;
